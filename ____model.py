...
class binNet(nn.Module):
    def __init__(self):
        ...
        self.vp_contract_1 = self.contracting_block(8)
        self.eb_contract_1 = self.contracting_block(32)
        self.rgb_contract_1 = self.contracting_block(32)
        self.merge_contract_1 = self.contracting_block(64)

        self.other_pooling = self.max_pooling(2,2)

        self.vp_contract_2 = self.contracting_block(16)
        self.eb_contract_2 = self.contracting_block(64)
        self.rgb_contract_2 = self.contracting_block(64)
        self.merge_contract_2 = self.contracting_block(128)

        self.merge_contract_3 = self.contracting_block(256)        

        self.merge_bottom_1 = self.bottom_layer(512)

        
        #DECODER
        self.expand_4 = self.expansive_block(768)
        self.expand_3 = self.expansive_block(800)
        self.expand_2 = self.expansive_block(400)
        self.crop_conv = self.reAdjustSize()
        self.final_conv = self.finalConv()


    def initial_block(self,ic):
        chanel_map = {2:4,7:16,3:16,12:32}
        oc = chanel_map[ic]
        res = nn.Sequential(
    ...
    def expansive_block(self,ic):
        ...
        if ic==400:
            res = nn.Sequential(
                nn.Conv2d(in_channels=ic,out_channels=192,kernel_size=3,stride=1,padding=1),
                nn.BatchNorm2d(192),
                nn.ReLU(),
        ...
        chanel_map = {1536:512,1088:512,768:512}
        oc = chanel_map[ic]
...
#############################################################################################
    ...
    #     # introduce supervison
        ...
        res = 10*res
        res = torch.clamp(res,max=10.)
        return res

    def scale_invariant_loss(self,pred, gt):
